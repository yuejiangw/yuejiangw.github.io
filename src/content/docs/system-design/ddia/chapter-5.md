...
**LLW 是 Cassandra 仅有的冲突解决方法**，它可以实现最终收敛的目标，但是以牺牲数据持久性为代价。如果同一个主键有多个并发写，即使这些并发写都向客户端报告成功，但最后只有一个写入值会存活，其他的将被系统默默丢弃。在一些场景如缓存系统，覆盖写是可以接受的。**如果覆盖、丢失数据不可接受，则LWW并不是解决冲突很好的选择**。
